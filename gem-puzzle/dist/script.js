...
var GameRules = /*#__PURE__*/function (_Render) {
  ...
  _createClass(GameRules, [{
    ...
    key: "calcEmptyCellCoord",
    value: function calcEmptyCellCoord(coord, blockSize) {
    ...
    key: "swapBlocks",
    value: function swapBlocks(firstBlock, secondBlock) {
      var t = this.numbers[firstBlock];
    ...
    value: function newGameHandler() {
      ...
      var leader = document.querySelector('.leader-wrap');
      leader.remove();
      overlay.remove();
      this.addTimer();
    }
    ...
    value: function winHandler() {
      ...
      var resultWrapper = document.querySelector('.leader-list');
      ...
      li.textContent = "Moves: ".concat(JSON.parse(localStorage.getItem('results'))[0][0], ". Time: ").concat(JSON.parse(localStorage.getItem('results'))[0][1]);
  ...
  return GameRules;
}(_render__WEBPACK_IMPORTED_MODULE_0__.default);
...
__webpack_require__.r(__webpack_exports__);
/* harmony import */ var _gameRules_js__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! ./gameRules.js */ "./src/gameRules.js");
;
_gameRules_js__WEBPACK_IMPORTED_MODULE_0__.default.init();
...
var Render = /*#__PURE__*/function () {
  ...
  _createClass(Render, [{
    key: "createIconHTML",
    value: function createIconHTML(iconName) {
    ...
    value: function getField() {
      ...
      var localStorageItem = localStorage.getItem('results');
      ...
      if (JSON.parse(localStorageItem)) {
        for (var i = 0; i < JSON.parse(localStorageItem).length; i++) {
          var li = document.createElement('div');
          li.textContent = "Moves: ".concat(JSON.parse(localStorageItem)[i][0], ". Time: ").concat(JSON.parse(localStorageItem)[i][1]);
      ...
      volumeUp.addEventListener('click', function () {
        _this.isVolumeOn = !_this.isVolumeOn;

        if (_this.isVolumeOn) {
          volumeUp.innerHTML = '';
          volumeUp.innerHTML = _this.createIconHTML('volume_up');
        } else {
          volumeUp.innerHTML = _this.createIconHTML('volume_off');
        }
      });
      ...
      this.fieldSizeOptions.forEach(function (item) {
    ...
    value: function blocksPosition(block, index) {
      ...
      if (block.innerHTML % this.rows === 0 && block.innerHTML !== 0) {
        var _backgrounTop = "-".concat(backTop * this.blockSize - this.MULTIPLIERS_FOR_BACKGROUND.top, "px");

        var _backgroundLeft = "-".concat(backLeft * this.blockSize + this.MULTIPLIERS_FOR_BACKGROUND.left, "px");

        block.style.backgroundPosition = " ".concat(_backgroundLeft, "  ").concat(_backgrounTop, " ");
      }
